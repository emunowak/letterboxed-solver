...
public class CharactersWordsValidator extends AbstractValidator<LetterWall, SolverInvalidConfigurationException> {
    ...
    protected LetterWall validateAndApplyNext( LetterWall letterWall ) throws SolverInvalidConfigurationException {
        if ( !letterWall.getWords().stream().allMatch( word -> word.matches( ONLY_LETTERS_PATTERN ) ) ) {
